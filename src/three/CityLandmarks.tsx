...
export default function CityLandmarks({
  ...
  return (
    <group>
      ...
      {/* Only landmarks that identify a sector get a label: a functional cue
          for wayfinding, not decoration. A landmark standing in for a company
          (ticker set) has no mesh here to label, and every other decorative
          landmark now relies on its own shape to be recognizable. */}
      {labels &&
        landmarks
          .filter((l) => l.sector)
          .map((l) => (

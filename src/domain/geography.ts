...
// Tokyo's sector towns: the shared sector identities, given this city's ground.
const townGround: Record<string, { x: number; z: number; width: number; depth: number }> = {
  technology: { x: -50, z: -40, width: 54, depth: 54 },
  financials: { x: -19, z: 20, width: 49, depth: 32 },
  healthcare: { x: 110, z: -57, width: 43, depth: 38 },
  consumer: { x: 14, z: -32, width: 48, depth: 38 },
  communications: { x: -54, z: -104, width: 45, depth: 28 },
  industrials: { x: 86, z: 65, width: 36, depth: 36 },
  staples: { x: -76, z: 21, width: 30, depth: 28 },
  energy: { x: 111, z: 130, width: 30, depth: 32 },
  utilities: { x: -78, z: 65, width: 29, depth: 29 },
  materials: { x: -126, z: 4, width: 26, depth: 32 },
  realestate: { x: -24, z: 91, width: 41, depth: 29 },
};
export const sectors: Sector[] = sectorIdentities.map((s) => ({
  ...s,
  ...townGround[s.id],
}));
